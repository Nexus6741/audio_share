...
## 功能概述
- 使用 UDP 广播发现同一局域网内的其他设备，自动剔除掉线节点。
- 选择一台设备作为接收端，其余设备可以将麦克风/系统声音推送到该设备，并可从列表看到哪些设备处于“接收中”。
- Flutter UI 支持 Android、Windows、macOS，底层使用 `dart:io` 套接字完成传输。
...
3. 将应用同时运行在局域网内的多台设备上：
   ...
   - 在其他设备的列表中选择该接收端，即会开始推流。

## 技术要点与下一步改进
- **音频采集**：使用 [`record`](https://pub.dev/packages/record) 的 `startStream` 获取 PCM 片段。部分平台需要手动开启麦克风权限；系统音频采集可通过平台通道接入原生实现。
- **传输协议**：当前使用 UDP 发送裸 PCM，简单低延迟但无重传；可替换为 WebRTC/QUIC 或 Opus 编码提升抗丢包能力。
- **播放**：`just_audio` 通过自定义 `StreamAudioSource` 播放每个收到的 buffer，可在生产环境中改为连续缓冲与抖动控制。
